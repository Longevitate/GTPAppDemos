...
import { MapPin, Heart, Clock, Navigation, ChevronLeft, ChevronRight } from "lucide-react";
import { useOpenAiGlobal } from "../use-openai-global";

// Widget version for cache busting
const WIDGET_VERSION = "4.1.0-inline-booking";
...
function TimeSlots({ location, apiBaseUrl }) {
  ...
  // Show only first 5 slots by default
          ...
          {locations.slice(0, 7).map((location, i) => (
            ...
            >
              <div
                ...
                className="flex w-full items-start hover:border-black/0! gap-4 py-4"
              >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-sm text-gray-600 italic mb-2">
                        {location.is_express_care ? 'ExpressCare Clinic' : ''} 
                        {location.is_urgent_care && !location.is_express_care ? 'Urgent Care Clinic' : ''}
                      </p>
                      ...
                      {location.rating_value && (
                        <div className="flex items-center gap-2 mb-2">
                          <div className="flex items-center gap-0.5">
                            {[...Array(5)].map((_, idx) => (
                              <Heart
                                key={idx}
                                className={`h-4 w-4 ${
                                  idx < Math.floor(parseFloat(location.rating_value))
                          ...
                          <span className="text-sm font-semibold">{location.rating_value}</span>
                          <span className="text-xs text-gray-500">| {location.rating_count} Ratings</span>
                        </div>
                      )}
                      
                      {/* Address */}
                      <p className="text-sm text-gray-700 mb-1">
                        {location.address_plain.split(",").slice(0, -1).join(",")}
                      </p>
                      ...
                      {location.hours_today && (
                        <p className="text-sm">
                          <span className="font-semibold">Open {location.hours_today.name}</span>
                          <br />
                          <span className="text-gray-600">
                            {location.hours_today.start} to {location.hours_today.end}
                          </span>
                        </p>
                      )}
                    </div>
